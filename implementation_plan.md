...
Dự án này là một ứng dụng đọc truyện offline được viết bằng **Flutter**. Ứng dụng đọc dữ liệu trực tiếp từ file JSON nội bộ (`assests/data/output.json`), hỗ trợ lưu lịch sử đọc (chương hiện tại, % tiến trình) và lưu các cấu hình giao diện đọc (cỡ chữ, màu nền, khoảng cách dòng, chủ đề tối mặc định).
...
### 1. Nguồn Dữ Liệu
* Sử dụng file `assests/data/output.json` có sẵn trong thư mục dự án.
...
* **Trang chủ**: Danh sách truyện trực quan. Sử dụng Logo mặc định từ `assests/image/logo` làm ảnh đại diện cho truyện, kèm thông tin tiến trình dạng thanh Progress Bar tinh tế (ví dụ: `Đang đọc: Chương 12 / 1464 (0.8%)`).
...
> [!IMPORTANT]
> Do thư mục tài nguyên của bạn được đặt tên là `assests` (chữ `s` ở giữa), chúng ta cần cấu hình chính xác trong file `pubspec.yaml` để tránh lỗi `AssetNotFoundException`:
> ```yaml
> flutter:
>   uses-material-design: true
>   assets:
>     - assests/data/output.json
>     - assests/image/
> ```
...
* **Mỗi Card truyện gồm**:
  * Ảnh mặc định (lấy từ `assests/image/logo`, thiết kế một hiệu ứng shadow bo viền lịch lãm).
...
* **Cài đặt cá nhân hóa**: Một nút hình bánh răng ⚙️ ở Header/Footer khi nhấn sẽ hiển thị **Bottom Sheet** tùy chỉnh:
  * *Màu nền & chữ*: Cung cấp 4 chủ đề (Tối Obsidian, Sáng Trắng, Vàng Sách Cổ, Xám Nhạt). Mặc định luôn là **Tối**.
  * *Cỡ chữ*: Nút tăng/giảm font chữ A- và A+.
  * *Khoảng cách dòng*: Tùy chọn dãn dòng để mắt không bị mỏi khi đọc lâu.
...
* Cấu hình thư mục tài nguyên đặc thù `assests/data/output.json` và `assests/image/logo` trong `pubspec.yaml`.
...
* Xây dựng `NovelProvider` để đọc dữ liệu từ file JSON nội bộ bằng `rootBundle.loadString('assests/data/output.json')`, parse JSON và lưu trữ vào bộ nhớ RAM.
